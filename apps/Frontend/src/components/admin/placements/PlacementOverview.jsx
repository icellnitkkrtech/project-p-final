    ...
    >
      <CardContent>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", mt: 1 }}>
          <Typography variant="body1">
            <strong>Role:</strong> {jobProfile?.designation}
          </Typography>
          <Typography variant="body1">
            <strong>Location:</strong> {jobProfile?.placeOfPosting}
          </Typography>
          <Typography variant="body1">
          ...
          {jobProfile?.stipend && (
            <Typography variant="body1">
              <strong>Stipend:</strong> ₹{jobProfile?.stipend}
            </Typography>
          )}
          <Typography variant="body1">
            <strong>Training Period:</strong> {jobProfile?.trainingPeriod}
          </Typography>
        </Box>
        ...
        <Typography variant="body1">
          <strong>Minimum CGPA:</strong> {eligibilityCriteria?.minCgpa}
        </Typography>
        <Typography variant="body1">
        ...
        <Typography variant="body1">
          <strong>Eligible Courses:</strong> {jobProfile?.course}
        </Typography>
        <Typography variant="body1">
          <strong>Eligible Branches:</strong>{" "}
          {eligibleBranchesForProfiles
            ?.map((profile) =>
              profile.branches?.[jobProfile?.course]
        ...
        <Typography variant="body1">
          <strong>Deadline:</strong> {applicationDetails?.applicationDeadline}
        </Typography>
        <Typography variant="body1">
          <strong>Link:</strong>
          <a
        ...
        {selectionProcess?.[0]?.rounds?.length > 0 ? (
          selectionProcess[0].rounds.map((round, index) => (
            <Typography key={index} variant="body1">
              <strong>Round {round.roundNumber}:</strong> {round.roundName} -{" "}
              {round.details}
            </Typography>
          ))
        ) : (
          <Typography variant="body1">
            No selection rounds specified.
          </Typography>
        )}
